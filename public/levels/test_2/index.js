...
const fullscreen_frames = g.frames([g_pick_screen]);
const door_frames       = g.frames([g_door]);
const opening_x_frames  = g.frames([g_exit_0, g_exit_1]);
const opened_x_frames   = g.frames([g_exit_2]);
const closing_x_frames  = opening_x_frames.slice().reverse();
const idle_1_frames     = g.frames([g_idle1_0, g_idle1_1, g_idle1_2]);

const walk_1_2_frames = [
	g.frame(g_idle1_0, 1/8, 128, -4), 
	g.frame(g_idle1_1, 1/8, 240, -2), 
	g.frame(g_idle1_2, 1/8, 373, 18), 
	g.frame(g_idle1_0, 1/8, 497, 38), 
	g.frame(g_idle1_1, 1/8, 609, 73), 
	g.frame(g_idle1_2, 1/8, 645, 175), 
	g.frame(g_idle1_0, 1/8, 609, 283), 
	g.frame(g_idle1_1, 1/8, 503, 363), 
	g.frame(g_idle1_2, 1/8, 355, 377), 
	g.frame(g_idle1_0, 1/8, 215, 360), 
	g.frame(g_idle1_1, 1/8, 115, 343), 
	g.frame(g_idle1_2, 1/8, 3, 313), 
	g.frame(g_idle1_0, 1/8, -74, 283), 
	g.frame(g_idle1_1, 1/8, -126, 242) 
];
const walk_2_1_frames = walk_1_2_frames.slice().reverse();

const walk_1_3_frames = [
	g.frame(g_idle1_0, 1/8, 65, 36), 
	g.frame(g_idle1_1, 1/8, 136, 75), 
	g.frame(g_idle1_2, 1/8, 186, 111), 
	g.frame(g_idle1_0, 1/8, 263, 143), 
	g.frame(g_idle1_1, 1/8, 337, 182), 
	g.frame(g_idle1_2, 1/8, 387, 212), 
	g.frame(g_idle1_0, 1/8, 412, 202) 
];
const walk_3_1_frames = walk_1_3_frames.slice().reverse();

const walk_2_3_frames = [
	g.frame(g_idle1_0, 1/8, -55, 240), 
	g.frame(g_idle1_1, 1/8, 21, 250), 
	g.frame(g_idle1_2, 1/8, 96, 245), 
	g.frame(g_idle1_0, 1/8, 165, 235), 
	g.frame(g_idle1_1, 1/8, 238, 227), 
	g.frame(g_idle1_2, 1/8, 325, 222), 
	g.frame(g_idle1_0, 1/8, 412, 202) 
];
const walk_3_2_frames = walk_2_3_frames.slice().reverse();

const walk_3_x_frames = [
	g.frame(g_idle1_0, 1/8, 454, 174), 
	g.frame(g_idle1_1, 1/8, 500, 120), 
	g.frame(g_idle1_2, 1/8, 550, 54), 
	g.frame(g_idle1_0, 1/8, 599, -14), 
	g.frame(g_idle1_1, 1/8, 607, -54) 
];
...
const sel_fullscreen = g.touch(fullscreen_shapes).stops(fullscreen);
const sel_windowed   = g.touch(windowed_shapes).stops(fullscreen);
const sel_2_1        = g.touch(sel_1_shapes).stops(idle_2);
const sel_3_1        = g.touch(sel_1_shapes).stops(idle_3);
const sel_x_1        = g.touch(sel_1_shapes).stops(idle_x);
const sel_1_2        = g.touch(sel_2_shapes).stops(idle_1);
const sel_3_2        = g.touch(sel_2_shapes).stops(idle_3);
const sel_x_2        = g.touch(sel_2_shapes).stops(idle_x);
const sel_1_3        = g.touch(sel_3_shapes).stops(idle_1);
const sel_2_3        = g.touch(sel_3_shapes).stops(idle_2);
const sel_x_3        = g.touch(sel_3_shapes).stops(idle_x);
const sel_1_x        = g.touch(sel_x_shapes).stops(idle_1);
const sel_2_x        = g.touch(sel_x_shapes).stops(idle_2);
const sel_3_x        = g.touch(sel_x_shapes).stops(idle_3);
const sel_x_x        = g.touch(sel_x_shapes);

const op_x     = g.touch(op_x_shapes).stops(opened_x);
const noop_x_1 = g.touch(sel_1_shapes).stops(opened_x, idle_x);
const noop_x_2 = g.touch(sel_2_shapes).stops(opened_x, idle_x);
const noop_x_3 = g.touch(sel_3_shapes).stops(opened_x, idle_x);
const noop_x   = g.touch(bg_shapes).stops(opened_x);
